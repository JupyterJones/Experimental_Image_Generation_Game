...
def query_llava(image_path, system_instruction):
    """
    Queries local Ollama LlaVa model with base64 encoded image.
    """
    import gc
        ...
        while current_frame < frames_current:
            ...
            try:
                ...
                if use_visual_director and (current_frame > 0) and (current_frame % visual_director_interval == 0):
                    ...
                    system_instruction = (
                        f"The original starting theme of this story is: '{original_starting_prompt}'.\n"
                        f"The current visual theme is: '{current_prompt}'.\n"
                        "Describe what you see in this image in one brief sentence.\n"
                        "Then, write a new Stable Diffusion prompt (under 50 words) that keeps the scene in context of the original starting theme and the current visual theme but evolves it slightly "
                        "(e.g., introducing a new space creature, changing lighting, or moving to a nearby room/planet).\n"
                        "Return ONLY the new prompt, with no intro, outro, or conversational filler."
                    )
                    new_p = query_llava(clean_path, system_instruction)
                    if new_p:
                        if new_p.startswith('"') and new_p.endswith('"'):
                            new_p = new_p[1:-1]
                        with state_lock:
...
<button class="btn-green" onclick="window.open('/media', '_blank')">
    MAKE VIDEOS
        ...
        <h3 style="margin-top:10px; margin-bottom:5px;">AI Visual Director (LLaVA)</h3>
