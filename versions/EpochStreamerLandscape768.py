...
OUTPUT_DIR = os.path.join(BASE_DIR, "static", "streamer2")
STATE_FILE = os.path.join(OUTPUT_DIR, "streamer2.json")
LOG_FILE_PATH = os.path.join(OUTPUT_DIR, "streamer2.txt")
...
active_caption = ""
caption_remaining = 0
current_prompt = "masterpiece, highly detailed image of an unworldly tropical jungle on an alien planet, giant tropical flowers, beautiful tropical folage, some insects and small animals, this scene is a small stone path surrounded by shrubs and tropical trees, colorful alien birds"
def logit(*args):
    try:
...
def draw_metadata_caption(img, frame_idx, total_frames, metadata, curr_zoom, curr_pan_x, curr_pan_y):
    try:
        ...
        return img
'''
def apply_border(img, border_path="static/border2.png"):
    """
...
<style>
    ...
    .thumb-strip img { width: 90px; height: auto; border: 2px solid #333; border-radius: 4px; opacity: 0.6; }
...
<body>
    <div class="column left">
        ...
        <label>Base Prompt</label><textarea id="prompt" rows="3">masterpiece, highly detailed image of an unworldly tropical jungle on an alien planet, giant tropical flowers, beautiful tropical folage, some insects and small animals, this scene is a small stone path surrounded by shrubs and tropical trees, colorful alien birds.</textarea>
    ...
    <script>
        ...
        setInterval(()=>{
            fetch('/status').then(r=>r.json()).then(d=>{
                ...
                if(d.history) document.getElementById('thumb_strip').innerHTML = d.history.map(i=>`<img src="/static/streamer2/${i}?t=${Date.now()}">`).join('');
...
SHARED_PATH = os.path.join(BASE_DIR, "static", "streamer2")
...
{% for v in videos %}
<div class="box">
<video controls src="{{ url_for('static', filename='streamer2/' + v) }}"></video><br>
...
{% for img in images %}
<div class="box">
<img src="{{ url_for('static', filename='streamer2/' + img) }}"><br>
...
<audio controls style="width:250px;">
    <source src="{{ url_for('static', filename='streamer2/' + mp3) }}" type="audio/mpeg">
</audio><br>
