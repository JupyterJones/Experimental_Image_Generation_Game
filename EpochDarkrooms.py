...
OUTPUT_DIR = os.path.join(BASE_DIR, "static", "darkroomsX")
STATE_FILE = os.path.join(OUTPUT_DIR, "darkroomsX.json")
LOG_FILE_PATH = os.path.join(OUTPUT_DIR, "darkroomsX.txt")
...
# moonbeam:latest seems to be hung up and finding urns that do not exist
...
def query_llava(image_path, system_instruction, model_name=None):
    """
    ...
    try:
        ...
        selected_model = model_name or visual_director_model or "LlaVa:latest"
        ...
        while current_frame < frames_current:
            ...
            try:
                ...
                if use_visual_director and (current_frame > 0) and (current_frame % visual_director_interval == 0):
                    ...
                    img_desc = query_llava(clean_path, system_instruction, model_name=visual_director_model)
                    if img_desc:
...
def generate_story():
    try:
        ...
        use_saved_script = req_data.get("use_saved_script", False)
        saved_diary = req_data.get("saved_diary", [])
        ...
        files = sorted([f for f in os.listdir(OUTPUT_DIR) if f.startswith("clean_") and not f.startswith("clean_base_") and f.endswith(".png")])
        if not files:
        ...
        if not files:
            return jsonify({"error": "No generated frames found in static/darkroomsX/ to process."}), 400
...
def preview_story():
    try:
        ...
        prompt = (
            "You are an expert psychological horror writer. You will write a survival diary of a traveler lost in the Backrooms.\n\n"
            "Below is a sequence of frames representing the traveler's journey. For each frame, you are given:\n"
            "1. Director Intent / Story Beat: The thematic concept, action, or progression that must happen.\n"
            "2. Emotional State: The traveler's current level of sanity and fear.\n"
            "3. Speech Inflection Style: How the text must be formatted to guide voice inflection (e.g., ellipses for pauses, exclamation marks for panic).\n\n"
            "Your task is to write a cohesive, continuous first-person story diary matching this sequence.\n"
            f"You MUST write exactly {total_frames} diary entries, one for each frame. "
            "Keep each entry brief (15-30 words).\n\n"
            "Format your output strictly using '[ENTRY]' as a separator before each frame entry, like this:\n"
            "[ENTRY] Entry 0 text...\n"
            "[ENTRY] Entry 1 text...\n\n"
    ...
    <script>
        ...
        setInterval(()=>{
            fetch('/status').then(r=>r.json()).then(d=>{
                ...
                if(d.history) document.getElementById('thumb_strip').innerHTML = d.history.map(i=>`<img src="/static/darkroomsX/${i}?t=${Date.now()}">`).join('');
...
SHARED_PATH = os.path.join(BASE_DIR, "static", "darkroomsX")
...
{% for v in videos %}
<div class="box">
<video controls src="{{ url_for('static', filename='darkroomsX/' + v) }}"></video><br>
...
{% for img in images %}
<div class="box">
<img src="{{ url_for('static', filename='darkroomsX/' + img) }}"><br>
...
<audio controls style="width:250px; display:block; margin: 0 auto 8px auto;">
    <source src="{{ url_for('static', filename='darkroomsX/' + mp3) }}" type="audio/mpeg">
</audio>
...
def compile_story_movie():
    ...
    if not wav_files:
        return "No narration WAV files found in static/darkroomsX/. Please generate your story first.", 400
    ...
    # Load backrooms_diary.json to get the tension level for each frame (for camera shake!)
    diary_map = {}
    diary_path = os.path.join(SHARED_PATH, "backroomsX_diary.json")
    if os.path.exists(diary_path):
        try:
            with open(diary_path, "r") as f:
...
def text_to_mp3():
    ...
    <h1>Text to MP3</h1>
    <form method="post">
    <textarea name="text" style="width:60%; height:400px; font-size:2vw;"></textarea><br>
    <select name="voice">
